  ...
  async function handleSendEmail() {
    ...
    try {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/send-sellsheet-email`, {
        ...
        body: JSON.stringify({ name, email, optIn, carName: "Cadillac LYRIQ Sport AWD", carUrl: "https://mothers-showtime.vercel.app/cadillac-lyriq" }),
      });
  ...
  async function handleQuickSend() {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/functions/v1/send-sellsheet-email`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${SUPABASE_ANON_KEY}` },
        body: JSON.stringify({ name, email, optIn: false, carName: "Cadillac LYRIQ Sport AWD", carUrl: "https://mothers-showtime.vercel.app/cadillac-lyriq" }),
      });
      if (!res.ok) throw new Error("Failed to send");
      setSent(true);
    } catch (e) {
          ...
          <div style={styles.saveSection}>
            ...
            {sent && (
              <div style={styles.successBox}>
                ...
                <p style={styles.successText}>Check your inbox — the Cadillac LYRIQ details are headed to {email}</p>
              </div>
            )}
          </div>
...
const styles = {
  ...
  sendBtn: { width: "100%", background: "#D4AF37", color: "#000", border: "none", borderRadius: 8, padding: "16px", fontSize: 16, fontWeight: 700, cursor: "pointer", fontFamily: "'Georgia', serif" },
  privacyText: { fontSize: 11, color: "#555", textAlign: "center", margin: "12px 0 0" },
  errorText: { fontSize: 13, color: "#ff4444", margin: "0 0 12px" },
  successBox: { background: "rgba(212,175,55,0.08)", border: "2px solid #D4AF37", borderRadius: 12, padding: "32px 24px", textAlign: "center" },
  successIcon: { fontSize: 32, color: "#D4AF37", margin: "0 0 8px" },
...
const css = `
  ...
  .send-btn:hover { background: #b8962e !important; }
  .form-input:focus { border-color: #D4AF37 !important; outline: none; }
